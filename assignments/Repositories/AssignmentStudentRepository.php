...
final class AssignmentStudentRepository {
	...
	public function get_last_attempt( $course_id, $assignment_id, $student_id ) {
		global $wpdb;

		return $wpdb->get_row(
			$wpdb->prepare(
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				"SELECT user_assignment_id, grade, status FROM $this->table_name WHERE course_id = %d AND assignment_id = %d AND user_id = %d ORDER BY updated_at DESC LIMIT 1",
				$course_id,
				$assignment_id,
				$student_id
			),
