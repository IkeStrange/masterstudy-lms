...
final class OrderRepository extends OrderAbstractRepository implements OrderInterface {
	...
	public function get_detailed_instructor_orders( $instructor_course_ids, $search, $length, $start, $columns, $order ) {
		...
		$wc_orders_table  = stm_lms_get_wc_orders( $this->db );
		...
		$sql = $this->db->prepare(
			...
			LEFT JOIN wp_postmeta pm1 ON wc.id = pm1.post_id AND pm1.meta_key = '_billing_first_name'
			LEFT JOIN wp_postmeta pm2 ON wc.id = pm2.post_id AND pm2.meta_key = '_billing_last_name'
		...
		if ( $order_column_index >= 0 && isset( $columns[ $order_column_index ]['orderable'] ) && 'true' === $columns[ $order_column_index ]['orderable'] ) {
			...
			if ( 'date' === $columns[ $order_column_index ]['data'] ) {
				...
				$sql .= " ORDER BY (SELECT COUNT(*) FROM wp_woocommerce_order_items WHERE order_id = wc.id) {$order_direction}";
		...
		foreach ( $orders as $order ) {
			...
			$total_price           = array_sum( array_map( fn( $course ) => (float) $course['price'], $courses_data ) );
			$formatted_total_price = 0 === $total_price
				? __( 'Free', 'masterstudy-lms-learning-management-system-pro' )
				: \STM_LMS_Helpers::display_price( $total_price );
	...
	private function get_search_filter_sql( $search ) {
		if ( is_numeric( $search ) ) {
			...
			if ( 2 === count( $search_parts ) ) {
				$sql .= $this->db->prepare(
					' EXISTS (SELECT 1 FROM wp_wc_order_addresses wa
				...
				$sql .= $this->db->prepare(
					' EXISTS (SELECT 1 FROM wp_wc_orders wo
							WHERE wo.id = o.ID AND wo.billing_email LIKE %s)',
					$search_like
				);

				$sql .= $this->db->prepare(
					' OR EXISTS (SELECT 1 FROM wp_wc_order_addresses wa
